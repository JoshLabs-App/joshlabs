...
function getChapterMarkup(chapter) {
  if (chapter.contentHtml) {
    return normalizePdfSourceHtml(chapter.contentHtml);
  }
...
const PDF_SENTENCE_END = /[。！？!?…]$/;
const PDF_PAGE_MARK = /^页码\s*[:：]?\s*\d+/;
const PDF_CHAPTER_TITLE = /^(第[一二三四五六七八九十百零〇两\d]+章\s*[^。！？\n]{1,32}?)(?=(?:当我|近年来|从多年|史尼克|你也喜欢|问得好|健康的|在讨论|让我|我们|从事|记得|一点|除了|想|这|在|对|有))/;
const PDF_FEATURE_POINTS = [
  "表达赞赏",
  "良好的沟通模式",
  "共聚时光",
  "委身于家庭",
...
function formatChapterTitle(text) {
  return String(text || "").replace(
    ...
    (_, chapter, rest) => (rest ? `${chapter} ${rest.trim()}` : chapter)
  );
}
...
function tidyPdfText(text) {
  return String(text || "")
    ...
    .replace(/&lt;/g, "<")
    .replace(/&gt;/g, ">")
    .replace(/(第[一二三四五六七八九十百零〇两\d]+章)\s+/g, "$1\u0000")
...
function isPdfFeaturePoint(text) {
  return PDF_FEATURE_POINTS.includes(text);
}

function isPdfHeading(text) {
  if (!text) return false;
  if (PDF_PAGE_MARK.test(text)) return false;
  if (isPdfFeaturePoint(text)) return false;
  if (/^[”』」'\"!?？！。、，,]/.test(text)) return false;
  if (/^第[一二三四五六七八九十百零〇两\d]+章/.test(text) && text.length <= 40) return true;
  if (PDF_SECTION_HEADINGS.includes(text)) return true;
  if (
    text.length <= 16 &&
    !PDF_SENTENCE_END.test(text) &&
    !/[，,；;]/.test(text) &&
    !/^[•·\d]/.test(text) &&
    /^[\u4e00-\u9fff]/.test(text) &&
    /[\u4e00-\u9fff]{2,}/.test(text)
...
function isPdfBullet(text) {
  return /^[•·]/.test(text) || /^\d+\.\s*\S/.test(text) || /^[A-Z]\.\s+\S/.test(text);
}

function peelPdfChapterTitle(text) {
  const match = text.match(PDF_CHAPTER_TITLE);
  if (!match) return null;
  const title = formatChapterTitle(tidyPdfText(match[1]));
  const body = tidyPdfText(text.slice(match[1].length));
  if (!title || !body || body.length < 12) return null;
  return { title, body };
}

function splitPdfMarkedPhrases(text) {
  const markers = [
    ...PDF_SECTION_HEADINGS.map((label) => ({ label, type: "h" })),
    ...PDF_FEATURE_POINTS.map((label) => ({ label, type: "point" })),
  ...
  while (remaining) {
    ...
    for (const marker of markers) {
      let from = 0;
      while (from < remaining.length) {
        const index = remaining.indexOf(marker.label, from);
        if (index < 0) break;
        const prev = index === 0 ? "" : remaining[index - 1];
        const next = remaining[index + marker.label.length] || "";
        const boundaryBefore = index === 0 || /[。！？!?…\n:：]/.test(prev);
        const boundaryAfter = !next || /[\u4e00-\u9fffA-Za-z“「《\n]/.test(next);
        const notMidPhrase = !/[的地得人们你我他她它]/.test(prev);
        if (boundaryBefore && boundaryAfter && notMidPhrase) {
          if (hitAt < 0 || index < hitAt) {
            hit = marker;
            hitAt = index;
          }
          break;
        }
        from = index + marker.label.length;
      }
    }
    if (hitAt < 0) {
      chunks.push({ type: "p", text: remaining });
      break;
    }
    const before = tidyPdfText(remaining.slice(0, hitAt));
    if (before) chunks.push({ type: "p", text: before });
    chunks.push({ type: hit.type, text: hit.label });
    remaining = tidyPdfText(remaining.slice(hitAt + hit.label.length));
  }
  return chunks;
}

function normalizePdfSourceHtml(html) {
  const lines = [...String(html).matchAll(/<p>([\s\S]*?)<\/p>/g)]
    .map((match) => tidyPdfText(match[1].replace(/<[^>]+>/g, "")))
    .filter(Boolean);
  ...
  const flush = () => {
    if (!buffer) return;
    const peeled = peelPdfChapterTitle(buffer);
    if (peeled) {
      blocks.push({ type: "h", text: peeled.title });
      for (const chunk of splitPdfMarkedPhrases(peeled.body)) blocks.push(chunk);
    } else {
      for (const chunk of splitPdfMarkedPhrases(buffer)) blocks.push(chunk);
    }
    buffer = "";
  };

  for (const rawLine of lines) {
    const line = tidyPdfText(rawLine);
    if (!line || PDF_PAGE_MARK.test(line)) continue;

    if (isPdfBullet(line)) {
      flush();
      blocks.push({ type: "p", text: line, className: "source-bullet" });
      continue;
    }

    if (isPdfFeaturePoint(line)) {
      flush();
    ...
    if (isPdfHeading(line)) {
      flush();
      blocks.push({ type: "h", text: formatChapterTitle(line) });
      continue;
    }

    buffer = joinPdfFragments(buffer, line);
    if (PDF_SENTENCE_END.test(line)) flush();
  }
  flush();
  ...
  for (let index = 0; index < blocks.length; index += 1) {
    ...
    if (block.type === "point") {
      const points = [];
      while (index < blocks.length && blocks[index].type === "point") {
        points.push(blocks[index].text);
        index += 1;
      }
      index -= 1;
      parts.push(
        `<ul class="source-point-list">${points
          .map((point) => `<li class="source-point">${point}</li>`)
          .join("")}</ul>`
      );
      continue;
    }
    if (block.type === "h") {
      parts.push(`<p class="source-heading">${formatChapterTitle(block.text)}</p>`);
      continue;
    }
    const classAttr = block.className ? ` class="${block.className}"` : "";
    parts.push(`<p${classAttr}>${block.text}</p>`);
  }

  return `<section class="section-block source-text">${parts.join("")}</section>`;
}
...
function renderCatalog() {
  ...
  chapters.forEach((chapter, index) => {
    ...
    button.innerHTML = `
      ...
      <strong>${formatChapterTitle(chapter.title)}</strong>
    `;
...
function render({ animate = false } = {}) {
  const chapter = chapters[currentIndex];
  fitSiteTitle();
  titleEl.textContent = formatChapterTitle(chapter.title);
